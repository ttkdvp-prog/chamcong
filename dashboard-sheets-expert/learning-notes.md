...
## 📱 5. Cải Tiến Tối Ưu Hóa Trải Nghiệm Di Động (Mobile UX/UI Optimization)

Để việc xem dữ liệu và nhập liệu trên điện thoại di động (iOS, Android) trở nên cực kỳ thuận tiện, nhanh chóng và không bị mỏi mắt hay bấm nhầm, hệ thống đã được triển khai các tiêu chuẩn thiết kế di động cao cấp sau:

1. **Bảng dữ liệu dạng thẻ (Card-based Table Layout)**:
   - Trên các màn hình di động hẹp (dưới `580px`), bảng lịch sử thô sơ (dễ bị tràn ngang) sẽ tự động biến đổi thành dạng **Danh sách Thẻ (Cards List)** cực kỳ sang trọng.
   - Mỗi hàng dữ liệu (`tr`) trở thành một chiếc thẻ Glassmorphism độc lập, bo tròn góc với hiệu ứng đổ bóng sâu.
   - Mỗi ô dữ liệu (`td`) được xếp dọc, hiển thị nhãn cột in đậm tương ứng bên trái (ví dụ: *HỌ VÀ TÊN*, *ĐIỂM*, *TIỀN THƯỞNG*,...) nhờ thuộc tính CSS `content: attr(data-label)` kết hợp thuộc tính HTML `data-label="..."` trong `app.js`. Điều này loại bỏ hoàn toàn việc phải vuốt ngang bảng, giúp theo dõi thông tin trực quan.
2. **Kích hoạt Bàn phím số tự động (Smart Keyboard Triggers)**:
   - Với các trường nhập kỳ thời gian (Tháng thưởng, Tháng giảm trừ, Tháng chỉnh sửa) có định dạng số `TTrrrr` (ví dụ: `62026`), việc mở bàn phím chữ cái mặc định và phải chuyển sang phím số rất mất thời gian.
   - Hệ thống được bổ sung thuộc tính **`inputmode="numeric" pattern="[0-9]*"`** cho các ô nhập liệu tháng. Khi người dùng chạm vào các ô này trên điện thoại, bàn phím số (Numeric Keypad) sẽ **tự động bật lên lập tức**, giúp nhập liệu nhanh chóng và an toàn.
3. **Diện tích chạm tối ưu (Touch Target Expansion)**:
   - Trên di động, các nút hành động Sửa và Xóa trong bảng được tự động mở rộng kích thước lên **`42px × 42px`** (đáp ứng tiêu chuẩn chạm ngón tay của Apple/Google) với khoảng cách giãn rộng `12px` để tránh tuyệt đối việc người dùng bấm nhầm giữa nút Sửa và nút Xóa.
4. **Bố cục KPI nén (Compact 2-Column Grid)**:
   - Để tránh việc 5 thẻ KPI chiếm trọn chiều cao màn hình điện thoại khiến người dùng phải cuộn trang liên tục, lưới chỉ số được nén thành **2 cột song song**. 
   - Riêng thẻ **Thực chi tiền thưởng** (chứa giá trị tiền tệ dài hơn) được tự động kéo giãn chiếm toàn bộ chiều rộng (2 cột) để hiển thị sắc nét và dễ đọc nhất.
5. **Form & Modal xếp dọc tự động (Form stacking)**:
   - Thay thế toàn bộ inline CSS chia 2 cột của Modal bằng class `.form-row-2col`. Class này tự động chia đôi cột trên máy tính, nhưng tự động xếp chồng dọc 1 cột trên điện thoại để các ô nhập liệu rộng rãi và dễ thao tác nhất.
